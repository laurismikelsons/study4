...
# mainīgie kas satur nasa api adresi un individuālo atslēgu
nasa_api_key = "???"
nasa_api_url = "https://api.nasa.gov/neo/"

# Getting todays date, lai varētu priecīzi pieprasīt info no NASA api galapunkta
dt = datetime.now()
...
# tiek veikts pieprasījums NASA api
...
# ja izdodas izveidot savienojumu https atgirež kodu 200 - var turpināt programmas darbu
if r.status_code == 200:

	#iegūtie json formāta dati tiek pārsieti
	json_data = json.loads(r.text)

	# masīvu mainīgie, kas saturēs asteroīdu objektus
	ast_safe = []
	ast_hazardous = []

	#pārbauda vai ir "element_count" objekts
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count'])
		print("Asteroid count today: " + str(ast_count))

		# ja šo elementu daudzums lielāks par 0 tad turpina
		if ast_count > 0:
			for val in json_data['near_earth_objects'][request_date]:
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					#mainīgie kas saturēs asteroīdu objektu vērtības
	...
	if len(ast_hazardous) > 0:
		# kārto asteroīdus pēc laika, kad pietuvosies
		...
		#kārto pēc attāluma kādā palidos garām zemei, atgriež to kas būs tuvākais
